...
//    }
    ...
    private void init() {
        ...
        if (DEBUG) {
            List<String> testDeviceIds = Arrays.asList("1601656296F82C8161C266B113767843");
...
//                rewardedLoad();
    ...
    public void interstitial(final boolean show, ProgressDialog pDl ) {
        ...
        if( mInterstitialAd == null ) { //init
            if (DEBUG) Log.e(TAG, "1. The interstitial ok dang load trước. "+JsonParams.getParam("admobid.android.interstitial"));
...
//                        Log.e(TAG, " onAdLoaded() = " + activity.getClass().getSimpleName() );
                    ...
                    @Override
                    public void onAdFailedToLoad(LoadAdError loadAdError) {
                        if (DEBUG) Log.e(TAG, "openapp LoadAdError is false: "+loadAdError.toString());
                        // Handle the error.
                        pDialog.dismiss();
                        if( activity.getClass().getSimpleName().equals("SplashScreen")) {
                            Intent intent = new Intent(activity, MainActivity.class);
